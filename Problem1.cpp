...
void convertStringToInt(vector<string> x, vector<string> y, vector<int> & new_x, vector<int> & new_y);
...
int main()
{
	...
	for (int i = 0; i < num_x.size(); i++) {
		x.insert(x.begin() + i, num_x.substr(i, 1));

	}

	cout << "Enter second binary number: ";
	cin >> num_y;
	for (int i = 0; i < num_y.size(); i++) {
		y.insert(y.begin() + i, num_y.substr(i, 1));

	}
	convertStringToInt(x, y, new_x, new_y);
	vector<int> sub;
	vector<int> r;
	makeSameSize(new_x, new_y);
	divide(new_x, new_y, sub, r);
	cout << "Quotient is: ";
	print(sub);
	cout << endl;
	cout << "Remainder is: ";
